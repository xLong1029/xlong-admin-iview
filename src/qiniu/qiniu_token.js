/*
 * 功能 : 七牛云上传凭证算法实现参考（引用的第三方CryptoJS库）
 * 作者 : 七牛云存储，罗永梅做部分修改
 * 日期 : 2017-9-18
 * 版本 : version 1.0
 */
...
export function genUpToken(accessKey, secretKey, bucketName) {
    ...
    var expire = 12;
    // 上传凭证有效截止时间
    policy.deadline = Math.round(new Date().getTime() / 1000) + expire * 3600;

    var put_policy = JSON.stringify(policy);
